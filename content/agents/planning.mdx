...
Planning tools give your agents the ability to map out their approach before diving into execution. Instead of figuring out what to do step-by-step in real-time (which can lead to blind alleys and wasted effort), an agent with planning tools can:
...
```json
{
  ...
  "steps": [
    {
      "id": 1,
      "description": "Search web for current pricing of top 5 competitors",
      "tools": ["web_search"],
      "status": "pending"
    },
    {
...
```json
{
  ...
  "status": "ready",
  "context": {
    "previous_step": "Found pricing pages for 4 of 5 competitors",
...
1. **Adding a step:** Web search reveals a 6th competitor that should be included
...
```json
{
  ...
  "step_id": 2,
  "result": "Successfully scraped pricing from all 5 competitors"
}
```
...
```
For complex tasks involving more than 3 steps, always start by creating a plan.
...
If the agent discovers that 3 leads have already been contacted, it updates the plan to skip those and find 3 more cold leads instead.
